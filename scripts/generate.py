...
def generate_single(config: dict, prompt: str = None, output_dir: Path = None):
    ...
    images = generator.generate(
        prompt=prompt, num_images=1, output_dir=output_dir or Path("gallery")
    )
...
def generate_collection(
    config: dict, prompts: list[str], collection_name: str, output_dir: Path = None
):
    ...
    for i, prompt in enumerate(prompts, 1):
        print(f"\n[{i}/{len(prompts)}] {prompt}")
        images = generator.generate(prompt=prompt, num_images=1, output_dir=output_path)
        if images:
...
Examples:
  ...
  python scripts/generate.py --mode custom --prompts-file my_prompts.txt
    ...
    parser.add_argument(
        "--num", type=int, default=5, help="Number of images for batch mode"
    )
    parser.add_argument(
        "--prompts-file",
        type=Path,
    ...
    parser.add_argument(
        "--output-dir", type=Path, help="Output directory (default: gallery/)"
    )
    parser.add_argument(
        "--config",
        type=Path,
