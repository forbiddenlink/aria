...
def train_lora(profile_name: str, profile: dict):
    ...
    if not check_dataset(dataset_path):
        ...
        print(f"   Download with: python scripts/download_specialized_datasets.py {profile_name}")
        return False
    ...
    cmd = [
        sys.executable,
        "-m", "ai_artist.training.train_lora",
...
def main():
    ...
    if args.list or not args.profile:
        ...
        for profile_name, profile in TRAINING_PROFILES.items():
            ...
            print(f"  Dataset: {profile['dataset_dir']}")
            print(f"  Output: {profile['output_dir']}")
            print(f"  Parameters: rank={profile['rank']}, steps={profile['max_train_steps']}")
            ...
            if check_dataset(dataset_path):
                ...
                print(f"  Status: ⚠️  Dataset not found")
                print(f"  Download: python scripts/download_specialized_datasets.py {profile_name}")
            print()
