...
class ImageGenerator:
    ...
    def load_model(self):
        ...
        # Fix for MPS black image issue: upcast VAE to float32 and disable attention slicing on MPS
        if self.device == "mps":
            logger.info("applying_mps_fixes", action="upcasting_vae_and_adjusting_settings")
            # Upcast VAE to float32 to avoid NaN values
            ...
            # Keep VAE slicing enabled - it helps with memory
            # self.pipeline.vae.enable_slicing() is already called above
